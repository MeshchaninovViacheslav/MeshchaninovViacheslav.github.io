...
\begin{equation}
\exp*{\mathbf{u}} \mathbf{x} = \cos(\|\mathbf{x}\|) \mathbf{u} + \sin(\|\mathbf{x}\|)
\frac{\mathbf{x}}{\|\mathbf{x}\|}, \quad \mathbf{u} \in \mathbb{S}^{d-1}, \ \mathbf{x} \in T*{\mathbf{u}}(\mathbb{S}^{d-1}),
\end{equation}

\begin{equation}
\exp\_{\mathbf{u}}^{-1}(\mathbf{v}) = \frac{\cos^{-1} \langle \mathbf{u}, \mathbf{v} \rangle}
...
{\sqrt{1 - \langle \bar{X}\_t, e_k \rangle^2}} \mathrm{d}t + \sigma_t \mathrm{d}B_t^d, \quad \bar{X}\_0 = x_0
\end{equation}
...
\begin{equation}
\eta*\theta(X_t, t) = \sum*{k=1}^d \left\langle p\_\theta(X_t, t), e_k \right\rangle \eta^k(X_t, t).
\end{equation}
